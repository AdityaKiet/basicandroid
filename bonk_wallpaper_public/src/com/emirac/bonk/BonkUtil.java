...
public class BonkUtil {
	...
	public static Bitmap imageFilePathToBitmap(Context c, String path,
		...
		try {
			...
			opts.inSampleSize = s;
			bmp = BitmapFactory.decodeStream(new FileInputStream(path), null,
					opts);
	...
	public static String uriToFilePath(Context context, String contentUri) {
		if (Uri.parse(contentUri).getScheme().equals("content")) {
			...
			Cursor cursor = context.getContentResolver().query(
					Uri.parse(contentUri), p, // which columns
	...
	public static String uriTitle(Context context, String contentUri) {
		...
		Cursor cursor = context.getContentResolver().query(uri, p, // which
																	// columns
				null, // which rows (all rows)
				null, // selection args (none)
				null); // order-by clause (ascending by name)
	...
	private static String fileUriTitle(Context context, String contentUri) {
		String result = null;
		String[] p = { MediaStore.MediaColumns.DISPLAY_NAME,
				MediaStore.MediaColumns.TITLE };
		Uri uri = Uri.parse(contentUri);
		String path = uri.getPath();
		String last = Uri.parse(path).getLastPathSegment();
		Cursor cursor = context.getContentResolver().query(
				MediaStore.Audio.Media.INTERNAL_CONTENT_URI, p, // which columns
				MediaStore.MediaColumns.DISPLAY_NAME + "='" + last + "'", // which
																			// rows
				null, // selection args (none)
				null); // order-by clause (ascending by name)
		if (cursor != null) {
